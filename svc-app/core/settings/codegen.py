...
is to emit the two language-agnostic backend artifacts the frontend codegen
consumes — the unified drf-spectacular OpenAPI schema and flows.json.

Hermetic by design: sqlite (in-memory) + local-memory cache, no postgres, no
redis, no broker. The sqlite constraint is architectural (session-log 2026-07-05):
emitting a schema never touches the database, so postgres is not needed. geo is
the documented exception (needs spatialite) and is not in the monolith module
set, so this instance stays pure-sqlite.
